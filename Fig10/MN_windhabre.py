...
re_arr = np.arange(0., 1.01, 0.1)
re_arr = re_arr[::-1]
re_arr = np.round(re_arr, 3)

h_arr = np.arange(0.1, 1.01, 0.1)
    ...
    for re in re_arr:
        ...
        for it in range(trials):
            ...
            for i_main in range(rounds):
                ...
                AdjMat = rewire.rewiring_process(AdjMat,
                                                 satA, re, probabs)
                ...
                CC_arr[reInd, hInd, it, i_main] = C_C
                DD_arr[reInd, hInd, it, i_main] = D_D
                CD_arr[reInd, hInd, it, i_main] = C_D

            print(np.mean(coopfrac_arr
                          [reInd, hInd, it, eqbTime:],axis=-1))
...
np.savez("heatmap_UniformMap_resVShab.npz",
         re = re_arr, h = h_arr,
         coopfrac = coopfrac_arr, sat = sat_arr,
         Dsat = Dsat_arr, Csat = Csat_arr, CC = CC_arr, CD = CD_arr,
         DD = DD_arr, Cdeg = Cdeg_arr, Ddeg = Ddeg_arr)
'''
