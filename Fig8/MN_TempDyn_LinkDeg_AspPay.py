...
for bet in beta_arr:
    ...
    for it in range(trials):
        ...
        for i_main in range(rounds):
            ...
            asp_arr[it, i_main-1] = np.sum(aspB[:,0])/len(aspB[:,0])
            pay_arr[it, i_main-1] = np.sum(pay[:,0])/len(pay[:,0])
    ...
    ax2 = plt.axes()
    ax1 = ax2.twinx()
    ax1.plot(trounds[eqbTime:rounds-1], SD_arr[0,eqbTime:rounds-1],
             'ro-', label='SD', alpha=0.5)
    ax1.plot(trounds[eqbTime:rounds-1], UC_arr[0,eqbTime:rounds-1],
             'go-', label='UC', alpha=0.5)
    ax1.plot(trounds[eqbTime:rounds-1], UD_arr[0,eqbTime:rounds-1],
             'o-', label='UD', c='peru', alpha=0.3)
    #ax1.plot(trounds[eqbTime:rounds-1], SD_arr[0,eqbTime:rounds-1],
    #'ro-', label='SD')
    plt.xlabel('rounds')
    ax2.plot(trounds[eqbTime:rounds-1], pay_arr[0,eqbTime:rounds-1],
             'bo--', label='Payoff', linewidth=1.8)
    ax2.plot(trounds[eqbTime:rounds-1], asp_arr[0,eqbTime:rounds-1],
             'go--', label='Aspiration', linewidth=1.8)
    ax1.legend(loc=(0.5,0.98), fontsize=15, framealpha=0, ncol=2)
    ...
    plt.savefig("SatactPayAsp_reNet_re"+str(Re)+"_h"+str(hab)+".tif")
    plt.show()    
    '''
    ...
    ax2 = plt.axes()
    ax1 = ax2.twinx()
    ax1.plot(trounds[eqbTime:rounds-1], pay_arr[0,eqbTime:rounds-1],
             'bo--', label='<pay>')
    ax1.plot(trounds[eqbTime:rounds-1], asp_arr[0,eqbTime:rounds-1],
             'go--', label='<asp>')
    plt.xlabel('rounds')
    ax2.plot(trounds[eqbTime:rounds-1],
             CC_arr[0,eqbTime:rounds-1], 'co-',
             label='C-C')
    ax2.plot(trounds[eqbTime:rounds-1],
             DD_arr[0,eqbTime:rounds-1], 'ro-',
             label='D-D')
    ax2.plot(trounds[eqbTime:rounds-1],
             CD_arr[0,eqbTime:rounds-1], 'ko-',
             label='C-D')
    ax1.legend(loc=(0.75,0.99), fontsize=15, framealpha=0)
    ax2.legend(loc=(-0.15,0.99), fontsize=15, framealpha=0, ncol=3)
    ax2.tick_params(axis='y', labelsize=15)
    ax1.tick_params(axis='y', labelsize=15)
    ax2.tick_params(axis='x', labelsize=15)
    ax1.grid(False)
    ...
    plt.subplots_adjust(right=0.9, left=0.15, bottom=0.13)
    plt.xticks(range(eqbTime, rounds-1, 3))    
    plt.savefig("AspPayLinks_reNet_re"+str(Re)+"_h"+str(hab)+".tif",
                dpi=300)
    plt.show()
    ...
    ax2 = plt.axes()
    ax1 = ax2.twinx()
    ax1.plot(trounds[eqbTime:rounds-1],
             coopfrac_arr[0,eqbTime:rounds-1],
             'co-', linewidth=1, label='C-frac')
    plt.xlabel('rounds')
    ax2.plot(trounds[eqbTime:rounds-1],
             Cdeg_arr[0,eqbTime:rounds-1], 'bo-',
             label='<deg(C)>')
    ax2.plot(trounds[eqbTime:rounds-1],
             Ddeg_arr[0,eqbTime:rounds-1], 'ro-',
             label='<deg(D)>')
    ax1.legend(loc=(0.85,0.99), prop={'size':15}, framealpha=0)
    ax2.legend(loc=(0,0.99), prop={'size':15}, ncol=2, framealpha=0)
    ax2.tick_params(axis='y', labelsize=15)
    ax1.tick_params(axis='y', labelsize=15)
    ax2.tick_params(axis='x', labelsize=15)
    ax1.grid(False)
    ...
    ax2.set_xlabel("round", fontsize=15)
    plt.subplots_adjust(right=0.9, left=0.15, bottom=0.13, top=0.93)
    ...
    ax2.plot(trounds[eqbTime:rounds-1],
             CD_arr[0,eqbTime:rounds-1], 'ko-',
             label='C-D')
    ax1.legend(loc=(0.85,0.99), fontsize=15, framealpha=0)
    ax2.legend(loc=(-0.15,0.99), fontsize=15, framealpha=0, ncol=3)
    ax2.tick_params(axis='y', labelsize=15)
    ax1.tick_params(axis='y', labelsize=15)
    ax2.tick_params(axis='x', labelsize=15)
    ...
    ax2.grid(False)
    ax2.set_xlabel("round", fontsize=15)
    plt.subplots_adjust(right=0.9, left=0.15, bottom=0.13, top=0.95)
    plt.xticks(range(eqbTime, rounds-1, 3))    
    plt.savefig("Links_reNet_re"+str(Re)+"_h"+str(hab)+".tif", dpi=300)
    plt.show()
    '''
